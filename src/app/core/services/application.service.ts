...
export class ApplicationService {
  ...
  constructor(
    ...
    this.user_subscription =this.$currentUser.subscribe((user) => {
      if (user && user !== null && user != this._currentUser) {
        this._currentUser = user;
        console.log('Calling the appSetup');
        this.setupAppData();
      } else {
  ...
  updateUserProfile(displayName: string, pictureUrl?: string): Promise<void> {
    ...
    if (this._currentUser && this._currentUser.publicProfile) {
      this._currentUser.publicProfile.name = displayName;
      if (pictureUrl !== undefined) {
        this._currentUser.publicProfile.profilePictureUrl = pictureUrl;
      }
      return this.usrSrv
        .updateUserProfile()
        .then(() => {
        ...
        .catch((error) => {
          this.notificationService.addNotification(
            this.i18nService.getTranslation('error.profile.update') +
              ': ' +
              error.message,
            'danger'
          );
      ...
      const errorMsg = this.i18nService.getTranslation(
  ...
  async uploadUserProfileImage(
    ...
    try {
      ...
      const imageRef = ref(
        storage,
        `users/${this._currentUser.id}/profile/${Date.now()}_${fileName}`
      );
  ...
  get active_activities(): Observable<number> {
    if (this._currentUser) {
      return collectionCount(
        collection(this.firestore, this._currentUser!.path, 'activities')
  ...
  resetPassword(email: string): Promise<void> {
    return this.usrSrv
      ...
      .catch((error) => {
        this.notificationService.addNotification(
          this.i18nService.getTranslation('error.password.reset') +
            ': ' +
            error.message,
          'danger'
        );
