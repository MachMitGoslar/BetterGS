...
export class ActiveTrackingBarComponent  implements OnInit {
  ...
  ngOnInit() {
    ...
    setInterval(() => {
      if (this._activeTracking && this._activeTracking.startDate) {
        this.elapsedTime = new Date().getTime() - this._activeTracking.startDate.getTime();
      }
    }, 100);
  }
  ...
  get activity(): Observable<Activity | void> {
    return this.$activeTracking.pipe(
      map(tracking => {
        if (tracking) {
          return tracking.activityRef ? this.applicationService.activityService.getActivityById(tracking.activityRef.id) : undefined;
        }
        return undefined;
      }),
