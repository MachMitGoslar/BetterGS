...
export function createServiceMocks(overrides: any = {}) {
  ...
  const mockUserService = {
    ...
    getAllUsersForRanking: jasmine.createSpy('getAllUsersForRanking').and.returnValue(
      of([MOCK_PUBLIC_PROFILE])
    ),
  ...
  const mockApplicationService = {
    $currentUser: userSubject.asObservable(),
    updateUserProfile: jasmine.createSpy('updateUserProfile').and.returnValue(Promise.resolve()),
    registerUserWithEmail: jasmine.createSpy('registerUserWithEmail').and.returnValue(Promise.resolve()),
    changePassword: jasmine.createSpy('changePassword').and.returnValue(Promise.resolve()),
    uploadUserProfileImage: jasmine.createSpy('uploadUserProfileImage').and.returnValue(
      Promise.resolve('https://example.com/uploaded-image.jpg')
    ),
    logout: jasmine.createSpy('logout').and.returnValue(Promise.resolve()),
    deleteAccount: jasmine.createSpy('deleteAccount').and.returnValue(Promise.resolve()),
  ...
  const mockActivityService = {
    ...
    updateActivity: jasmine.createSpy('updateActivity').and.returnValue(Promise.resolve()),
  ...
  const mockTrackingService = {
    startTracking: jasmine.createSpy('startTracking').and.returnValue(Promise.resolve()),
    stopTracking: jasmine.createSpy('stopTracking').and.returnValue(Promise.resolve()),
    getCurrentSession: jasmine.createSpy('getCurrentSession').and.returnValue(null),
    getTrackingSessions: jasmine.createSpy('getTrackingSessions').and.returnValue(
      of(MOCK_TRACKING_SESSIONS)
    ),
  ...
  const mockI18nService = {
    getTranslation: jasmine.createSpy('getTranslation').and.callFake((key: string) => {
      const translations: { [key: string]: string } = {
        ...
        'profile.validationError': 'Please fix the form errors before saving.',
        'profile.userDataMissing': 'User data not available. Please try again.',
        ...
        'profile.imageTooLarge': 'Image file is too large. Please choose a smaller file.',
        'profile.invalidImageType': 'Invalid image type. Please choose a JPEG, PNG, GIF, or WebP image.',
        ...
        'activities.noActivitiesDescription': 'Create your first activity to get started',
        ...
        'activities.createError': 'Failed to create activity. Please try again.',
        'activities.updateError': 'Failed to update activity. Please try again.',
        'activities.deleteError': 'Failed to delete activity. Please try again.',
...
export function createIonicMocks(overrides: any = {}) {
  ...
  const mockModalController = {
    create: jasmine.createSpy('create').and.returnValue(Promise.resolve(mockModal)),
    dismiss: jasmine.createSpy('dismiss').and.returnValue(Promise.resolve()),
    getTop: jasmine.createSpy('getTop').and.returnValue(Promise.resolve(mockModal)),
  ...
  const mockActionSheetController = {
    create: jasmine.createSpy('create').and.returnValue(Promise.resolve(mockActionSheet)),
...
export function createCapacitorMocks(overrides: any = {}) {
  ...
  const mockFilesystem = {
    readFile: jasmine.createSpy('readFile').and.returnValue(Promise.resolve({
      data: 'mock-file-data'
    })),
  ...
  // Network Mock
  const mockNetwork = {
    getStatus: jasmine.createSpy('getStatus').and.returnValue(Promise.resolve({
      connected: true,
      connectionType: 'wifi'
    })),
...
export function createTestingEnvironment(options: {
  ...
  const providers = [
    ...
    { provide: 'ApplicationService', useValue: serviceMocks.mockApplicationService },
    ...
    { provide: 'NotificationService', useValue: serviceMocks.mockNotificationService },
    ...
    { provide: 'ActionSheetController', useValue: ionicMocks.mockActionSheetController },
    { provide: 'ToastController', useValue: ionicMocks.mockToastController },
    { provide: 'LoadingController', useValue: ionicMocks.mockLoadingController },
 ...
 */
export function triggerObservableUpdate<T>(subject: BehaviorSubject<T>, value: T): void {
  subject.next(value);
}
